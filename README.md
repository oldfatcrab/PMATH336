...
- Prop: Let $G_1$ and $G_2$ be 2 groups such that $G_1 \simeq G_2$. Then
	...
	- (3) $G_1$ is cyclic iff $G_2$ is cyclic
- Proof of Prop:
	- Since $G_1 \simeq G_2$, exists an isoorphism $\phi:G_1\rightarrow G_2$. In particular, $\phi$ is not only a homomorphism, but it is also a bijection so that $G_2 = \phi(G_1)$. Also, $\phi^{-1}: G_2\rightarrow G_1$ is a bijective homomorphism and $G_1 = \phi^{-1}(G_2)$
...
- **Lemma**: Let $G_1$ and $G_2$ be groups, and $\phi:G_1\rightarrow G_2$ is a homomorphism.
	...
	- (2) If $G_1$ and $G_2$ are both cyclic of order $n$, then $\exists$ isomorphism $\phi:G_1\rightarrow G_2$ with $\phi(a)=b$ where $a$ is any generator of $G_1$ and $b$ is any generator of $G_2$.
	- Proof:
		...
		- (2) If $G_1 = \langle a\rangle$ and $G_2 = \langle b\rangle$. Set $\phi(a) = b$ and define $$\phi:G_1=\{e, a,\cdots, a^{n-1}\}\rightarrow G_2 = \{e,b,\cdots,b^{n-1}\}$$ $$a^m\mapsto b^m$$ Then, $\phi$ is clearly a bijection and also a homomorphism since $$\phi(a^m\cdot a^{m'}) = \phi(a^{m+m'}) = b^{m+m'} = b^m\cdot b^{m'} = \phi(a^m)\cdot \phi(a^{m'})$$ $\Rightarrow\phi$ is an isomorphism
- Prop: Let $G = \langle a\rangle$ be a cyclic group.
	- (1) If $|G| = \infty$, then $G\simeq \mathbb Z$
	- (2) If $|G| = n$, then $G\simeq \mathbb Z_n$
	- Pf:
		- (1) The homorphism $$\phi: G\rightarrow \mathbb Z = \langle a\rangle$$ $$a\mapsto 1$$ is an isomorphism. Moreover, $$\psi:\mathbb Z = \langle 1 \rangle =\rightarrow G = \langle a\rangle$$ $$1\mapsto a$$ $\leadsto \psi(m) = a^m$ because want $\psi$ to be homomorphism. 
			- **$\psi$ is 1 to 1** Suppose that $\psi(m) = \psi(m')\Leftrightarrow a^m = a^{m'}$. But $|a|$ is infinite because it generates G  which is inifinite. So, if $a^m = a^{m'}$, must have that $m = m'\Rightarrow \psi$ is 1:1
			- **$\psi$ is onto** $\forall x\in G, x=a^m$ for some $m\in \mathbb Z$ so that $x=\psi(a)\Rightarrow \psi$ is bijection homomorphism therefore $\psi$ is an isomorphism
			  
		- (2) The homorphism $$\phi: G\rightarrow \mathbb Z_n = \langle \overline a\rangle$$ $$a\mapsto \overline 1$$ is an isomorphism.
